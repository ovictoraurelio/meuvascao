...
8. **Acessibilidade e identidade**: texto mínimo 12 px (0.75rem), alvos 44 px, contraste 4,5:1; cores e fontes só por `var(--…)` de `design/tokens.json` (a partir da fatia F2). Exceção: assets estáticos em `public/` (favicon, símbolo) usam os hex da paleta. Nenhum escudo do clube ou de adversário; adversário em texto. Estados vazios honestos: nenhum número inventado.
...
10. **Ambientes**: o build resolve o ambiente por `CLOUDFLARE_ENV` (`npm run build:preview` / `build:production`); um `wrangler deploy --env` sobre um build sem essa variável publica variáveis de desenvolvimento. Deploy é sempre humano, a partir de `dist/server/wrangler.json`.
